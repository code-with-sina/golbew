<script setup>
    import write from '../../Layouts/Auth.vue';
        ...
        <div class="w-full px-4">
          <div
            ...
            <form class="px-2" @submit.prevent="submit()">
              ...
              <TextInput placeholder="Name" v-model="form.name" type="text" :message="form.errors.name" />
              <TextInput placeholder="Email" v-model="form.email" type="email" :message="form.errors.email" />
              <TextInput placeholder="Password" v-model="form.password" type="password" :message="form.errors.password" />
              <TextInput placeholder="Confirm Password" v-model="form.password_confirmation" type="password" :message="form.errors.password_confirmation" />

              <div class="mb-9">
                <button type="submit" :disabled="form.processing"
                  class="w-full px-5 py-3 text-base text-white transition duration-300 ease-in-out border rounded-md cursor-pointer border-primary bg-primary hover:bg-blue-dark">Sign Up </button>
              </div>
            </form>
            <span class="relative block text-center z-1 mb-7">
              ...
              <span class="relative z-10 inline-block px-3 text-base bg-white dark:bg-dark-2 text-body-secondary">OR</span>
            </span>
            
            <Link :href="route('forgotpassword')" class="inline-block mb-2 text-base text-dark dark:text-white hover:text-primary dark:hover:text-primary">
              Forget Password?
            </Link>
            ...
            <div>
              <span class="absolute top-1 right-1">
                <svg width="29" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
