                        ...
                        <table class="min-w-full bg-white">
                            ...
                            <tbody class="text-gray-700">
                                <tr v-for="item in data" :key="data.id">
                                    ...
                                    <td class="text-left py-3 px-4"><a class="hover:text-blue-500">{{ item.status }}</a></td>
                                    <td class="text-left py-3 px-4"><Link class="hover:text-blue-500 mx-2" as="button" :href="'edit/' + item.id">Edit</Link><Link class="hover:text-blue-500" as="button" :href="route('makePublish')" method="post" :data="{ status: 'published', id: item.id }">Promote</Link></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <p v-else class="pt-3 text-gray-600">
                        There is no unpublished Post yet!
                    </p>
                </div>
</template>